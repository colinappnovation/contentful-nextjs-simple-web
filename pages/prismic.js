				...
				{props.results.length > 0 ? (				
					...
					props.results.map((p) => {						
						...
						return (<Post
							...
							blogger={{fullname: data.fullname[0].text, jobtitle: data.jobtitle, profilepicture: data.profilepicture}}
							key={p.id}
						/>)
					})
				) : null}	
...
HomePage.getInitialProps = async () => {
	const posts = await client.query(
		...
		{ orderings: '[my.post.date desc]', 'fetchLinks' : ['author.fullname', 'author.jobtitle', 'author.profilepicture']}
	  )
	return posts
}
